...
describe('modules-event-log', () => {
  testIf(process.env.DB_TYPE === "postgres", 'postgres - event log creation', async () => {
    ...
    try {
      ...
      expect(results[0].row_id).toBe(1);
  ...
  testIf(process.env.DB_TYPE === "postgres", 'postgres - mapping tables', async () => {
    const config: CoreConfig = {
      name: 'data-test',
      slices: [dataModule, coreModule, eventLogModule, roleUpsertModule, fieldHashModule],
      clone: true,
    ...
    const context = await createContext(core, undefined, undefined, undefined, true);
    let options = buildOptions(context, {});

    try {
      const role = await Role.create({ name: 'fail' }, options);
      const site = await Site.create({ name: 'site1' }, options);
      const siteRole = await SiteRole.create({ roleId: role.id, siteId: site.id }, options);
